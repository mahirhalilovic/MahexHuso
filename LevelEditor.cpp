...
void LevelEditor::ConstructButtons() {
    // Menu buttons
    m_buttonMenuTile = Button(1220, 20, 100, 50, "TILES");
    m_buttonMenuMechanical = Button(1350, 20, 100, 50, "MECHANICAL");
    m_buttonMenuPlayer = Button(1480, 20, 100, 50, "PLAYER");

    // Tile buttons
    m_buttonTile = Button(1250, 120, 200, 50, "TILE");
    m_buttonSpikes = Button(1250, 190, 200, 50, "SPIKES");
    m_buttonCoin = Button(1250, 260, 200, 50, "COIN");

    // Mechanical buttons
    m_buttonKeydownBlock = Button(1250, 120, 200, 50, "KEYDOWN BLOCK");
    m_buttonPressureBlock = Button(1250, 190, 200, 50, "PRESSURE BLOCK");
    m_buttonHorizontal = Button(1250, 300, 125, 50, "HORIZONTAL");
    m_buttonVertical = Button(1425, 300, 125, 50, "VERTICAL");

    // Player buttons
    m_buttonMahexStart = Button(1250, 120, 200, 50, "MAHEX START");
    m_buttonMahexEnd = Button(1250, 190, 200, 50, "MAHEX END");
    m_buttonHusoStart = Button(1250, 260, 200, 50, "HUSO START");
    m_buttonHusoEnd = Button(1250, 330, 200, 50, "HUSO END");

    // Footer buttons
    m_buttonCancel = Button(1220, 650, 100, 50, "CANCEL");
    m_buttonEdit = Button(1350, 650, 100, 50, "EDIT");
    m_buttonSave = Button(1480, 650, 100, 50, "SAVE");

    m_buttonCancel.SetFont("i pixel u", 26, false);
    m_buttonEdit.SetFont("i pixel u", 26, false);
    m_buttonSave.SetFont("i pixel u", 26, false);
    m_buttonTile.SetFont("i pixel u", 26, false);
    m_buttonMahexStart.SetFont("i pixel u", 26, false);
    m_buttonMahexEnd.SetFont("i pixel u", 26, false);
    m_buttonHusoStart.SetFont("i pixel u", 26, false);
    m_buttonHusoEnd.SetFont("i pixel u", 26, false);
}
    ...
    if(m_buttonSave.IsMouseOver(mousePos.x, mousePos.y)) {
        ...
        if(ShowSaveFileDialog(filePath)) {
            if(SaveToFile(filePath)) {
                MessageBox(NULL, "Level saved successfully!", "Success", MB_OK);
        ...
        if(MessageBox(m_hwnd, "Do you want to exit level editor?", "Exit", MB_OKCANCEL) == IDOK)
            shouldExitToMainMenu = true;
        return;
...
bool LevelEditor::SaveToFile(const std::string& filename) {
    try {
        ...
        if(m_mahexStart.x == -1 || m_mahexEnd.x == -1 || m_husoStart.x == -1 || m_husoEnd.x == -1) {
            MessageBox(m_hwnd, "Start and end positions must be defined!", "Error", MB_OK);
            return false;
        }
...
void LevelEditor::LoadFromFile(const std::string& filename) {
    ...
    if(!file.is_open()) {
        MessageBox(NULL, "Failed to open level file!", "Error", MB_OK);
        return;
    }
    ...
    for(const auto& tileData : levelData["tiles"]) {
        int x = tileData["x"].get<int>() / 48;
        int y = tileData["y"].get<int>() / 48;
        TileType type = StringToTileType(tileData["type"].get<std::string>());

        m_grid[x][y].m_posX = tileData["x"].get<int>();
        m_grid[x][y].m_posY = tileData["y"].get<int>();
        m_grid[x][y].m_type = type;

        switch(type) {
            case TileType::MAHEX_START:
                m_mahexStart.x = tileData["x"].get<int>();
                m_mahexStart.y = tileData["y"].get<int>();
                break;
            case TileType::MAHEX_END:
                ...
                break;
            case TileType::HUSO_START:
                m_husoStart.x = tileData["x"].get<int>();
                m_husoStart.y = tileData["y"].get<int>();
                break;
            case TileType::HUSO_END:
            ...
            case TileType::KEYDOWN_BLOCK:
            case TileType::PRESSURE_BLOCK:
                m_grid[x][y].m_id = tileData["id"].get<int>();
                if(m_currentID < m_grid[x][y].m_id) m_currentID = m_grid[x][y].m_id;
                break;
            case TileType::KEYDOWN_PLATE:
                m_grid[x][y].m_id = tileData["id"].get<int>();
                if(m_currentID < m_grid[x][y].m_id) m_currentID = m_grid[x][y].m_id;
                m_grid[x][y].m_orientation = StringToOrientation(tileData["orientation"].get<std::string>());
                break;
            case TileType::PRESSURE_PLATE_START:
                m_grid[x][y].m_id = tileData["id"].get<int>();
                if(m_currentID < m_grid[x][y].m_id) m_currentID = m_grid[x][y].m_id;
                m_grid[x][y].m_orientation = StringToOrientation(tileData["orientation"].get<std::string>());
                m_grid[x][y].m_movement = StringToOrientation(tileData["movement"].get<std::string>());
                m_grid[x][y].m_endPos = tileData["endPos"].get<int>();
                if(m_grid[x][y].m_movement == Orientation::VERTICAL) {
    ...
    if(IsKeyPressed(VK_ESCAPE)) {
        if(!m_escapePressed) {
            ...
            if(m_mechanicalTileInProgress) {
                ...
                m_selectedTile = TileType::NONE;
            } else if(MessageBox(m_hwnd, "Do you want to exit level editor?", "Exit?", MB_OKCANCEL) == IDOK) shouldExitToMainMenu = true;
        }
    } else {
